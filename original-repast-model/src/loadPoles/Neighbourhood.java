...
public class Neighbourhood {
	...
	public Neighbourhood(Context<Object> context, String fileName, int humanCount) {
		...
		try {
			...
			gridX = Files.lines(p).map((String x) -> x.length()).max((i, j) -> i.compareTo(j)).orElse(10);

			GridFactory factory = GridFactoryFinder.createGridFactory(null);

			GridBuilderParameters<Object> gridParameters = new GridBuilderParameters<Object>(new StrictBorders(),
					new SimpleGridAdder<Object>(), true, gridX, gridY);
